...
contract ReverseRegistrar {
     ...
     */
    constructor(ECNS ecnsAddr, NameResolver resolverAddr) public {
        ecns = ecnsAddr;
        defaultResolver = resolverAddr;

        // Assign ownership of the reverse record to our deployer
        ReverseRegistrar oldRegistrar = ReverseRegistrar(ecns.owner(ADDR_REVERSE_NODE));
    ...
    function claimWithResolver(address owner, address resolver) public returns (bytes32) {
        ...
        address currentOwner = ecns.owner(node);

        // Update the resolver if required
        if (resolver != address(0x0) && resolver != ecns.resolver(node)) {
            // Transfer the name to us first if it's not already
            if (currentOwner != address(this)) {
                ecns.setSubnodeOwner(ADDR_REVERSE_NODE, label, address(this));
        ...
        if (currentOwner != owner) {
            ecns.setSubnodeOwner(ADDR_REVERSE_NODE, label, owner);
        }
